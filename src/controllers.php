...
$app->get('/output-success', function () use ($app) {
            ...
            $lang = $app['session']->get('current_language') ? $app['session']->get('current_language') : 'es';
            return $_COOKIE['result_showed'] == "false" && isset($_COOKIE['user_latitude']) && isset($_COOKIE['user_longitude']) ? $app['twig']->render('result.html.twig', array('language' => $lang)) : $app->redirect($app['url_generator']->generate('homepage'));
        })
        ->bind('result')
;
